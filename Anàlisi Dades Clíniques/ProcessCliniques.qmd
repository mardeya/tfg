...
clinical_data_p <- clinical_data %>% 
  select(TCIA_ID, OS, hepatitis, age, Sex, Smoking, Alcohol,
         fhx_can, fhx_livc, Diabetes, `Personal history of cancer`, Evidence_of_cirh, Tr_Size, tumor_nodul, `Vascular invasion`, Metastasis, Lymphnodes,`Portal Vein Thrombosis`, T_involvment, AFP)
```
...
```{r}
nombre_archivo <- "Clinical Data csv/clinical_data_processed.csv"
...
```{r}
nombre_archivo <- "Clinical Data csv/clinical_data_processed_test.csv"

write.csv(clinical_data_p_test, file = nombre_archivo, row.names = FALSE)

nombre_archivo <- "Clinical Data csv/clinical_data_processed_train.csv"
